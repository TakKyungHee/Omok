import omokenv
from omokalgorithm import OmokAlgorithm
import os
import numpy as np
import math
import random
...
if __name__ == '__main__':
    ...
    optimizer_1p = optim.Adam(policy_net_1p.parameters(), lr=LR, amsgrad=True)
    # optimizer_2p = optim.Adam(policy_net_2p.parameters(), lr=LR, amsgrad=True)
        ...
        for t in count():
            ...
            if not done:  # player 2 승리 시 한 번 더 반복되지 않기 위해
                if terminated:  # player 1 승리
                    ...
                    # player 2
                    action_2p = select_action(
                        state_2p, policy_net_2p, eval=True)
...
#         torch.load('model_1p.pth', map_location=device))
#     print('loaded')
# policy_algorithm = OmokAlgorithm(1)
...
#             else:
#                 # player 1
#                 action_1p = select_action(
#                     state_1p, policy_algorithm)
#                 observation_1p, reward_1p, terminated = env.step(action_1p)
...
#                     # player 2
#                     action_2p = select_action(
#                         state_2p, policy_algorithm, eval=True)
#                     observation_2p, reward_2p, terminated = env.step(action_2p)
